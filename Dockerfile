...
WORKDIR /frontend
COPY --chown=redash redash/package.json redash/yarn.lock redash/.yarnrc /frontend/
COPY --chown=redash redash/viz-lib /frontend/viz-lib
...
COPY --chown=redash redash/client /frontend/client
COPY --chown=redash redash/webpack.config.js /frontend/
...
COPY redash/requirements_all_ds.txt ./
RUN if [ "x$skip_ds_deps" = "x" ] ; then pip install -r requirements_all_ds.txt ; else echo "Skipping pip install -r requirements_all_ds.txt" ; fi

COPY redash/requirements_bundles.txt redash/requirements_dev.txt ./
RUN if [ "x$skip_dev_deps" = "x" ] ; then pip install -r requirements_dev.txt ; fi

COPY redash/requirements.txt ./
